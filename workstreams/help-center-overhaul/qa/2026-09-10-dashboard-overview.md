...
**Settled in the standard, September 11, 2026.** Tara asked what was actually best for Fin, and the answer turned out to favor the change for a better reason than the reading-experience argument that prompted it. Every failure mode golden question 6 names is a dangling reference; it is a rule against pointing backward, not a requirement to teach every prerequisite. Naming the surface a step happens on satisfies it. And how to reach a surface is a fact like any other, so it gets one canonical home, which in an Overview article is the orientation section. Repeating it inside a later procedure is the duplication the say-once rule exists to prevent.

The rule is now in `docs/standards/content-standards.md` under "Naming a surface is not teaching navigation", with a resolution note in `docs/standards/golden-questions.md` under "How the house standard resolves factor 6 against factor 9". No scorecard change was needed: Chunk independence check 1 already reads "No section depends on 'above', 'then', or the section before it", which is the dangling-reference test, so the check set stays at revision 2.
...
**Reviewer comments, September 15, 2026.** Kelly Miragliotta left 15 comments on the Notion page. Tara is working through them one at a time, and this section records each as it is settled.

- **"tile" in the widget definition.** Kelly suggested "section might be a better word". Tara rejected "section", since the Dashboard's sections are its widget groupings rather than the widgets themselves, and proposed dropping the noun instead. The sentence now reads "Each Dashboard widget displays a specific type of information. Together they give you a quick view across several areas of Perchwell." An intermediate version kept "that" and left the first sentence a fragment with no main verb; dropping "that" fixes it. A purely functional definition is consistent with how the Hot Sheet is defined in Add a Hot Sheet to the Dashboard, which says what one shows rather than what shape it is on screen. The heading echo holds: the lead still opens on "Each Dashboard widget" under "What each Dashboard widget shows", so Retrieval signals check 4 is unaffected and the score stays at 100.0
- **Naming the task in the Hot Sheets link sentence.** Kelly commented "I would add creating them to this list as that is key" on "Add a Hot Sheet to the Dashboard walks through both types step by step". The comment is anchored to that sentence rather than to the two type bullets above it, so "this list" means what the sentence names as the linked article's contents. It now reads "walks through creating both types step by step". Beyond the reading-experience point, no form of "create" appeared anywhere in the Hot Sheets section, so a member asking Fin how to create a Hot Sheet had no lexical match to retrieve that section on. It also brings the sentence in line with the Tags section, which already reads "For creating a Tag, sharing one with a client, and setting its alerts, see Tags Page Overview". Adding the widget is named by the link text itself, so the shorter form loses nothing. Not adopted: a third bullet in the type list, since creating is not a third type and a third bullet would imply a third section behind it, which Retrieval signals check 7 tests for. The score stays at 100.0
- **Tara's manual edits, synced from Notion.** Four changes made directly on the Notion page and brought back into the repo draft. "so you see what has changed without running individual searches" lost its closing clause, which Kelly flagged as factually wrong: the Dashboard's Saved Searches widget does run individual searches. The Hot Sheets lead went from "tracks new and updated listings" to "tracks new and updated listing activity". The two Hot Sheet bullets were swapped, so the saved search hot sheet now comes first. And "may know Tags as carts" became "may know Tags as listing carts"
- **An MLS announcement page added to the integrations examples.** Kelly commented "add something like 'or an MLS announcement page'" on the generic-examples sentence. The sentence now reads "Your MLS may connect others, such as tools for public records and tax data, market analytics, or an MLS announcement page". The suggestion is sourced: `baldwin/integration-tools-on-the-dashboard.md:62` carries an "MLS announcements" category holding BR Announcements, and `baldwin/key-workflow-changes.md:39` describes it as a website reached through an integration link on the Dashboard. Kelly's generic phrasing is better here than naming BR Announcements, since it carries to a CRMLS row without edit, and it sits in the sentence about what else an MLS may connect rather than in the bulleted list of common integrations. The score stays at 100.0
- **Two knock-on fixes from Tara's edits and Kelly's comment.** Changing the Hot Sheets lead to "listing activity" left the phrase twice in one sentence, so "which means you see the latest listing activity without rerunning a search" became "which means you do not have to rerun a search to see it". The clause keeps its "which means" construction, which Answer completeness check 3 looks for on system terms, and keeps the member as the subject. Separately, adding an MLS announcement page to the integrations examples contradicted the section's own framing: an MLS's announcement page is the MLS's own site, not a third party. The heading is now "Launch integrations from the Dashboard" and its lead "Launch an integration from the Integrations panel, which sits on the right side of the Dashboard and holds the tools and pages your MLS connects to Perchwell". Nothing is lost by dropping "third-party", because the section already states "The integration opens outside Perchwell". The heading echo is stronger than before, since the lead now opens on the heading's own verb. The score stays at 100.0
**Full-article review, September 15, 2026.** Run after Kelly's comments were all addressed. The gate passes 14 of 14. One scored check was failing and three unscored weaknesses were found; all four are fixed, so the article is back at 100.0.

**The article was at 98.0, not 100.0, from September 10 until this review.** Accuracy and confidence check 5, member-controlled verbs, was failing on "Click Manage widgets to pick which widgets appear". That is the same construction Tara ruled on when she set the rule: she proposed "customize which widgets appear" for the opening paragraph and chose "display" instead. The sweep that followed changed the opening and never touched the body, so the article contradicted itself, line 29 against line 92, and disagreed with Customize Your Dashboard, which uses "display" in all three of its own places. Now "pick which widgets display".

This is the second time a check was added to the set and the article was not re-swept against it, after the September 11 unmarked-guess correction. The lesson is the same both times: adding a check does not rescore the articles already written, and the rescore has to be a deliberate step.

Three unscored fixes, none of which any check would have caught:

- **The opening paragraph's fourth outcome named nothing in the article.** It read "access the workflows available from the Dashboard" while the section is "Launch integrations from the Dashboard" and the description says "access integrations". The word "workflows" appeared nowhere else. The third outcome also under-promised, covering choosing but not arranging, though the section does both. The list now reads "understand what each widget shows, choose and arrange which widgets display, and launch integrations from the Dashboard", so all four outcomes echo the headings they point at. Retrieval signals check 7 passed either way, since every item did have a section behind it, which is the limit of what that check tests
- **The integrations section stated the same fact twice.** "which means the panel is the accurate list of what you have" and, six lines later, "Open the Integrations panel to see the full set for your account". The second sentence was cut. Chunk independence check 6 tests across sections, not within one, so this was invisible to it
- **"timeframe" against "time frame".** Dashboard Overview spells it as one word; Add a Hot Sheet to the Dashboard spells it as two, in four places. Both are locally defensible: the Dashboard Overview line points at Dashboard FAQ, which uses one word, and the Hot Sheet steps name the Edit hot sheet modal's field. The mirror is split roughly evenly, so there is no house precedent. Left open for Tara to settle from the modal's on-screen label, after which both articles follow it
