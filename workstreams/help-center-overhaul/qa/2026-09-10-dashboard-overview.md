...
1. **The widget lineup differs by MLS, but less than first thought.** Investigated in full on September 10, 2026; see `audit/dashboard-widget-lineup-2026-09-10.md`. All five widgets this article covers, Hot Sheets, Contacts, Saved Searches, Tags, and Listings, exist in CRMLS as well, so the five widget sections are universal as written. CRMLS and NYC additionally carry a Presentations widget and one or two Days on Market widgets, which Baldwin has no trace of. The recommendation is to keep the five as the spine and point at Manage widgets for anything MLS-specific, the same move the integrations section makes.

   **An earlier version of this file said CRMLS has no Tags widget. That was wrong.** It came from `crmls/the-dashboard-page-overview.md`, which omits Tags, whereas `crmls/customizing-your-dashboard.md`, the article that owns the Manage Widgets enumeration, lists it. An overview article's silence is not evidence a feature is absent
