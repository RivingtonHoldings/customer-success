...
4. **"off-market" is spelled two ways.** The Hot Sheet widget labels the row Off market, two words, confirmed by screenshot and already recorded in the Add a Hot Sheet QA file from September 15. **Resolved September 22, 2026 by Tara:** corrected in the Add a Hot Sheet draft. `Dashboard Overview` still carries "off-market" in its delegation line and is live in Intercom, so it needs a rung-3 update through `/port-to-intercom` rather than an edit here. Nine other uses of "off-market" across the mirror are the ordinary real-estate sense, not the widget row, and stay as they are.
