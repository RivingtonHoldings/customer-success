...
**The mirror is not the set.** It holds what has been ported to Intercom, and while the overhaul is running most siblings are still Notion drafts, so the mirror's copy of a migrated sibling is its pre-migration text. Before any set-level check, query the new database for rows sharing the article's `Collection` and read those drafts:
...
On September 22, 2026 the Dashboard FAQ migration ran its set pass against the mirror alone. Four of the six Dashboard articles were unported drafts, which produced a recommendation to fix something already fixed and hid a near-verbatim duplication with Add a Hot Sheet to the Dashboard.
...
2. **Diff the sources.** When there is both an old Notion body and a live mirror body, list every fact, step, bullet, and tip that appears in one and not the other, or reads differently. The live mirror wins on facts because it is what members and Fin see today, not because of its date. A newer Notion page does not outrank it: edited later is not the same as verified, and a row marked `Transfer to Intercom` records an intention rather than a fact. Where the two conflict and no third source resolves it, ask rather than breaking the tie by date or by majority. The old Notion page supplies the video link, the properties, and the click script as a hint for the article's outcomes. Show the differences to the user before drafting unless they said "just do it"; never merge them silently. **Where the two sources agree, that is not verification.** Documents written from the same screen go stale together, so agreement reads as corroboration and is not. Before drafting an article that documents a screen's controls, their labels, or what a control opens, list the product questions you would otherwise mark `[confirm: ...]` and ask them up front with the source differences. A teammate at their desk answers in minutes what a marker takes a review cycle to resolve, and the answer usually contradicts both documents rather than picking one.
...
5. **Choose the article type and title.** Workflow, Overview, or FAQ, per `docs/standards/content-standards.md`. Take the skeleton from `.claude/skills/article-draft/references/article-template.md` and model the result on `.claude/skills/article-draft/references/example-article.md`. When the old title is a bare noun ("Universal Search Bar"), propose a task-led title per the naming standard and offer the old one as the alternative; the user picks. Record the old title in the report either way. **Run the collision check on any title before proposing it:** grep the mirror for the distinctive words in it (`grep -ril "<phrase>" docs/help-center/*/`), check them against the collection names in the mirror frontmatter, and against the page names in `docs/product-context.md`. A title that repeats another article's name, a Perchwell page, or a collection puts two articles in competition for the same query. Report any hit with the proposal rather than leaving the user to spot it.
...
7. **Self-review.** Run the 14 golden questions and list every one that fails with the fix or the question for the user. Then run the quality checklist at the end of `docs/standards/content-standards.md`. Then grep the draft for em dashes, "you can", "allows you to", "able to", the verb "use" outside the accepted exceptions, "MLS number", "MLS #", and the marketing do-not list. Then grep for `**` and confirm every hit is a callout label or the `**Description:**` line; nothing else in the body is bold. Then run the verb pass: grep for "appear", "appears", "display", and "displays", and for each hit confirm the member is acting on the product rather than the screen acting back, as in "adjust which widgets display" over "change which widgets appear"; a sentence whose subject is genuinely the system, such as "the widget updates automatically", is correct as written. In the same pass, check possessive density: where "your" appears more than twice in one sentence or repeats before every item in a list, let one "your" govern the run, as in "your contacts, saved searches, tags, and listings" rather than "your contacts, your saved searches, your tags, and your listings", and drop it where it earns nothing, as in "near the upper left" rather than "near the upper left of your screen". Keep it where it separates the member's own things from everyone else's ("your own listings", "your MLS permissions"), and never touch it inside a linked article's exact title. Then run the plain language pass: grep the draft for the plain language do-not list in `docs/standards/content-standards.md`, read every hit in context rather than trusting the grep, and keep the ones the list does not actually ban, as in "public records" and "return to the Dashboard". "tiles" in alt text is no longer among them: the exception was struck on September 18, 2026, and "tile" is banned in alt text as well as in the body, except where it is the on-screen label. Check alt text punctuation in the same pass, since it is the same field: alt text ends with a period, a numbered step does not. Then read the description, the opening paragraph, and every heading as an agent would, and name any word an agent would not say out loud. Ask two questions of every term that reads as product vocabulary, not one: is the word on screen, and would a member use it when the control is not in front of them? A term that passes the first and fails the second belongs at the moment the member has to find the thing and nowhere else, so name the label once there and say what the control does everywhere else. Before swapping any term, check it against the terminology rules in `docs/product-context.md` and against the on-screen label; "modal", MLS ID, Saved Search, Hot Sheet, and timeframe are settled and stay. In the same pass, confirm that no sentence restates the `Steps:` block in the abstract, and that the verb a member would type for the task appears in the section that covers it rather than only inside a link's title. Then run the list-order pass: take every enumeration in the opening paragraph and in each section's lead sentence, and confirm the sections that follow run in the same order and that every listed item has a section behind it. Then run the redundancy pass: list every distinct fact in the draft and confirm each appears in exactly one section, collapsing any enumeration that shows up twice. Then, when the article belongs to a set that covers one surface, run the set-consistency pass, assembling the set from the database rows sharing the article's `Collection` and their draft bodies, not from the mirror alone: the feature name is spelled and pluralized the same way in every article in the set, the opening's verb matches the verb in the section headings it promises, any ordered pair runs in the order of the controls in the product, and the small conventions match: hyphenation of position words, and whether bullet lead-in labels take a period or a colon. Settle each by counting across the set rather than by preference. Then check the load-bearing claims: a claim that sets up the article is written into the description, the opening paragraph, and a section lead at once, so confirm all three against the same source rather than spot-checking one. That shape is how an unsourced claim survives a review, because fixing the sentence you noticed leaves the other two. Last, check that no sentence claims every member has the feature, and that no availability claim sits next to a `[confirm: ...]` marker that undercuts it. Fix everything before scoring.
8. **Score the rewrite.** Same scorecard, then the delta per dimension and in total. Run both gates: the 14 golden questions and the plain language gate in `docs/standards/fin-readiness-scorecard.md`. Report them separately, never blended; a plain language failure is reported with every term flagged and its replacement, not as a bare verdict. Write the five to eight Fin test questions and mark, for each, whether the old and the new article answer it from one section.
9. **Save the draft** to `workstreams/help-center-overhaul/outputs/drafts/YYYY-MM-DD-<slug>-rewrite.md`. Top of file: the mirror frontmatter, plus `source_notion_url: <old page url>` and `migrated_from: master-article-list` for a migration. Then a blank line and the metadata comment `<!-- Article Status: Draft | MLS/AOR: <value> | Collection: <value> | Roles: <value> | Video Included: <Yes or No> | Visuals Included: <Yes or No> | Notion: <page url once created> | Old title: <old title, if changed> -->`, then the article. Never edit the mirror file itself; only `/sync-help-center` writes there.
...
12. **Report.** Old title and new title; before-and-after score, band, and delta per dimension; golden questions that still fail and why; the source differences and which side won; the property mapping and any decision taken; open `[confirm: ...]` items; the Notion URL; and what remains: peer review in Notion, then `Ready to Transfer`, then `/port-to-intercom`, which converts the body and moves each image's alt text into Intercom's `alt` attribute. Fin labels per `docs/standards/fin-labeling.md`, MLS audience, and publishing stay manual, because the connector cannot write tags or audiences. The full checklist is under "After publishing" in `.claude/skills/article-draft/references/notion-publishing.md`.
...
Apply these eleven patterns when rewriting. `references/transformation-patterns.md` has before-and-after examples of each.
...
Replace generic descriptions like "allows you to stay connected by storing information" with concrete actions: "Filter by Recently Created or New." Name the actual UI elements the reader will see, spelled the way the screen spells them and left unbolded, per pattern 9.
...
Name the real things, not the category that contains them, and use the word an agent would say out loud. This is a retrieval rule as much as a readability one: members type the words they know, so a word an agent would not say is a word Fin cannot match them on. Change the headings too, since the heading is what Fin retrieves on.

| Was | Now |
|---|---|
| "It is a lookup tool"; "Each lookup opens one record" | "lookup" gone; the task named instead |
| "record", in the heading, the lead, and the bullets | "listing, agent, or contact", the three real things |
| "What the Universal Search Bar returns" | "What you find with the Universal Search Bar" |
| "Partial entries return close matches, which means ..." | "You do not have to type the whole thing, which means ..." |
...
- **port-to-intercom**: the next step after this one. Once a person has reviewed the draft in Notion and moved the row to `Ready to Transfer`, it carries the article into Intercom and sets the row to `Live in Intercom`. This skill never writes to Intercom itself.
- **sync-help-center**: refresh the mirror before a migration if it is more than a week old.
- **humanizer**: for removing AI-generated writing patterns. Run it after this skill and before scoring, not only when the output feels stilted; stiltedness is exactly what the writer is worst placed to notice, and it was the defect in all four articles migrated in September 2026. Re-run step 7 afterward, since the humanizer reintroduces "you can", which is a scored failure under Accuracy and confidence check 5.

  **Apply** patterns 3 to 5 (superficial -ing endings, promotional language, vague attributions), 7 (AI vocabulary), 9 to 11 (negative parallelisms, rule of three, synonym cycling), 13 (em dashes), and 22 to 25 (filler, hedging, generic conclusions, over-claimed certainty about people).

  **Overridden by the house standard**, because they pull the other way: its "Personality and soul" section, which asks for opinions, asides, and "I"; pattern 15, which collapses the `- **Label.** text` bullet lead-ins that transformation pattern 9 explicitly keeps; and pattern 16, which sentence-cases headings the naming standard requires in title case.
