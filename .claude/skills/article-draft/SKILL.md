...
6. **Self-review.** Run the 14 content readiness factors in `docs/standards/qa-checklist.md` against the draft and list every factor that fails, with the fix or the question for the user; do not invent limits, roles, or numbers to satisfy a factor. Then run the quality checklist at the end of `references/style-rules.md` and grep the draft for em dashes, "you can," "allows you to," "able to," the verb "use," and the marketing do-not list. Fix everything before saving.
...
10. **Report.** State the decision, the saved path(s), the Notion URL(s), the content readiness factors that fail and why, the open `[confirm: ...]` items, and the manual CS steps that remain: peer review in Notion, transfer to Intercom as a draft, public URL, Fin label, MLS audience, publish.
...
**Content readiness.** Every draft passes the 14 factors in `docs/standards/qa-checklist.md` (self-descriptive references, alt text on images, terms defined on first use, real lists for steps, question-form section headings, self-contained sections, audience and permissions stated, key entities repeated, one topic per section, intro sentences before tables, a jobs-to-be-done opening, complete instructions including what happens after the last step, explicit limitations and workarounds, exact numbers). Fin retrieves sections in isolation, which means each section has to stand on its own.
