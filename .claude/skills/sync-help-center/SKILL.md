...
Article bodies come from the public help center page for published articles (`support.perchwell.com/<mls>/en/articles/<id>`) and from `get_article` for drafts, which have no public page. The script fetches public pages in small batches with backoff and resumes if interrupted.

**The mirror is a rendering, not the stored article.** A published body is captured from the page Intercom serves a reader of that help center, and Intercom rewrites parts of it on the way out. Cross-links are the known case: a link stored as `support.perchwell.com/en/articles/...` is served to a Baldwin reader as `support.perchwell.com/baldwin/en/articles/...`, so the mirror shows the MLS path for a link the article does not contain. **Never diff the mirror against a Notion draft to decide what a port will change.** Call `get_article` and diff against the stored HTML. On 2026-09-23 a Dashboard Overview port was talked out of a false regression that way: seven links looked like they would be downgraded from `/baldwin/en/` to `/en/`, and the stored article had carried `/en/` all along. The mirror stays the right tool for searching, cross-linking, and reading what a member sees; it is the wrong tool for anything that compares against what Intercom stores.

One consequence worth knowing: the linking standard asks CS to flag default help center links at transfer, and the mirror hides exactly those links behind the rewritten path. A reviewer reading the mirror will not see them.
