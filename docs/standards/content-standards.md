...
- **Check the title against what already exists before settling it.** A title that repeats another article's name, a page in Perchwell, or a collection name puts two articles in competition for the same query, and Fin has to choose between them. Search the mirror for the distinctive words in the proposed title, and check them against the collection list. "Find and Manage Listings in the Listings Widget" was withdrawn on September 16, 2026 for exactly this: Manage Listings is a page reached from the main menu, it has its own article, and that article also covers filtering listings to find the ones you need. Two surfaces, one set of words. Where two articles genuinely cover similar jobs on different surfaces, each title names its own surface and the two link to each other.
...
- **Two or more named things get a labeled list.** When a section introduces two or more named controls, tabs, views, options, or states, name them in a bullet list with the label first, a colon, then one sentence on what each does. Prose is for one thing; a list is for a set. `- Filters: Set and adjust your search criteria.` beats "Filters is where you set your criteria, and Results is where the matching listings appear." The test is whether the section is introducing a set: where there is no set, prose is still right, and a section that names one control and links out does not need a list. Added September 24, 2026, after the Search Page Overview migration, where the reviewer made this same conversion in four of nine sections and left the other five as prose.
- **Bullet lead-in labels take a colon, not a period, and no bold.** `- Keep: Shows only the selected listings and hides the rest.` This was previously settled per article by counting across the set; it is now settled once. The no-bold rule still governs the label itself, per Formatting below.
...
- **Write the word a member would type.** Name the real things, not the category that contains them: "listing, agent, or contact", not "record". A busy agent does not say "record", "lookup", or "returns", so those words cost the reader a beat and cost Fin a match. This is a retrieval rule as much as a readability one: members search and type the words they know, in the help center and in Perchie, and the article has to contain them. The test is to read the sentence aloud as if to an agent on the phone. Added September 15, 2026, from Kelly's review, whose reason was retrieval: "they will search and type into Perchie those words they know and we want to make sure we show the right answer."
- **Check a candidate before swapping it.** A plain-language pass that changes a settled term works against its own goal. Before replacing any word, check it against the terminology rules in `docs/product-context.md` and against the on-screen label. "Modal" reads like developer vocabulary and stays, because one consistent word is what lets a member and Fin track the same thing. The same holds for MLS ID, Saved Search, Hot Sheet, timeframe, and any exact on-screen label, however awkwardly it reads in prose.
- **The exact-label rule reaches controls, not readouts.** Ask first whether the thing on screen is something the member has to find and act on. A button, filter, menu item, toggle, or navigation target is a control, and its label is spelled exactly as the screen spells it, however awkwardly it reads in prose. A status row in a chart, a column heading, a count, or any other label the member reads but never clicks is not a control, and there the word a member would type wins over the screen's rendering. The Hot Sheet widget labels its row Off market, and the Dashboard FAQ was briefly written that way and reverted: members type "off-market", the live article's own heading used it, and nine other uses across the mirror agree. Consistency across the help center beats matching one chart row's sentence case. Alt text runs the other way and names what the screen shows, because it stands in for an image someone cannot see. This and the rule below are the same question asked at two depths: Scope by is a control, so its label binds but only where the member reaches for it; Off market is not a control, so its label does not bind at all. Added September 23, 2026, from the Dashboard FAQ migration.
- **An on-screen label licenses the control, not the vocabulary.** Two questions decide a term, not one. Is the word on screen, and would a member use it when the control is not in front of them? A word that passes the first and fails the second belongs at the moment the member has to find the thing, and nowhere else. The Listings widget's menu is headed Scope by, so an article names Scope by once, where the member is looking at it, and everywhere else says what the control does: "choose whose listings you see", not "choose the listing scope"; "the button in its top right", not "the scope button". An agent looking at a button that reads MLS Listings does not know the word for it is scope. This is the mirror of the modal rule above, same check and opposite answer, which is why both are written down. Added September 16, 2026, after the Listings widget migration passed the plain language gate on "scope" and Tara caught it by reading as an agent would.
- **Cut a sentence that restates the Steps block in the abstract.** The section lead says what the feature is for, and what to reach for instead; the mechanics belong in the numbered steps. "It is a lookup tool: type what identifies the record, click the match, and the record opens" was three abstractions describing a procedure that sat twelve lines below it, and the reviewer's comment was "what does this mean?"
- **Name the task in the section that covers it.** The verb a member would type belongs in the body, not only inside a link's title. "Add a Hot Sheet to the Dashboard walks through both types step by step" left no form of "create" anywhere in that section, so a member asking Fin how to create a Hot Sheet had nothing to match on.
- Where two phrasings are equally accurate and equally clear, take the tighter one, stopping short of anything that reads as formal or stiff. "Reorder them into your preferred layout" beats "reorder them into the layout you want". This is the short-sentence rule applied at the phrase level, and the tighter form is often the product's own: the live article says "your preferred order". **Precision outranks tightness every time.** An exact on-screen label, a hedge that matches its source, or a clause that blocks a wrong reading all stay, even when longer. "# of Properties" stays over the smoother "number of Properties" because it is what the control reads; "statuses such as" stays over the shorter "every status" because the source hedged. Where cutting words would shift emphasis or drop a fact, propose the tighter version rather than making the swap silently. Two corollaries: **prefer the product's own wording** where the product has one, since "custom market segment" is verbatim from the setup card and a draft that adds an "a" the source does not have is further from the product, not closer to it; and **a contrast names the thing that actually contrasts**, so "Unlike the checkboxes" beats "Unlike other widgets" when the Listings widget is also an other widget and behaves the same way. Shorter for being precise. Added September 14, 2026; corollaries added September 15, 2026.
...
- **A plainly stated benefit is not a marketing adjective.** The list above bans praise, not usefulness. "Together they give you a quick view across several areas of Perchwell" and "which means you do not have to rerun a search to see it" both earn their place: each states what the member gets, in plain words, without an adjective doing the work. Added September 15, 2026, because the do-not list above reads broadly enough to suppress them.

**Plain language do-not list.** Each term is banned in one sense, not as a word. The replacement is the point; a term crossed out with nothing put in its place is not a fix.

| Do not write | Write instead |
|---|---|
| record, entry, entity, object, as a stand-in for the thing | the real noun: listing, agent, contact, Saved Search, Hot Sheet |
...
- **A newer Notion page does not outrank the live article.** Being edited more recently means it was written later, not that it was verified, and an old row marked `Transfer to Intercom` records an intention rather than a fact. Counting sources does not settle a disagreement either. Where the live article and the Notion page conflict and no third source resolves it, the conflict is a question for product, not a tie broken by date or by majority. Added September 14, 2026, after three factual errors in the Dashboard pair traced to exactly that move: the live article was right that the Listings widget cannot be reordered, against four sources that mentioned only removal, and right that the Manage Widgets panel holds four widgets, against two sources that said five.
- **The mirror is not the set.** `docs/help-center/` holds what has reached Intercom. While the overhaul is running, most sibling articles are Notion drafts, so a set-consistency check run against the mirror compares the new article to text the team has already replaced. Assemble the set from the Perchwell Help Center Database [Sep 2026] rows that share the article's `Collection`, and read their drafts. Added September 22, 2026, after the Dashboard FAQ migration compared itself to five pre-migration articles and missed a duplication with a finished draft.
- **Agreement between sources is not verification.** Two documents written from the same screen go stale together, and when they agree they read as corroboration. On the Listings widget migration the live article and the old Notion page both listed a Recently Added filter the product no longer has, both omitted Active Under Contract which it does have, and both called the Add/Edit form "the listing management form". One screenshot settled all three. Where an article documents a screen's controls, its labels, or what a control opens, ask for a current screenshot before drafting and treat what the documents agree on as the question rather than the answer. The existing source-precedence rules above govern what to do when sources disagree; this one governs the case where nothing disagrees and everything is wrong. Added September 16, 2026. Awaiting Kelly and Rafe.
...
- Title does not repeat another article's name, a Perchwell page name, or a collection name
...
- Every set of two or more named controls, tabs, views, or options is a labeled bullet list, not prose, with the label first and a colon
...
- Where two wordings are equally accurate, the tighter one is used, and no exact label or hedge was cut to get there
- No marketing adjectives from the do-not list, and no term from the plain language do-not list in the sense it bans
- The article names the real things rather than a category standing in for them, and every term that reads as jargon was checked against `docs/product-context.md` before it was changed or kept, and no on-screen label has been adopted as the article's own noun beyond the point where the member has to find the control
- Labels the member reads but never clicks, such as a status row or a column heading, use the word a member would type rather than the screen's rendering; alt text is the exception
- No sentence restates the Steps block in the abstract, and the verb a member would type appears in the section that covers that task
...
- For an article documenting a screen's controls, the labels and behavior were checked against a current screenshot, not only against the documents
