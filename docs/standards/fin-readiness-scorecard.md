...
1. No term from the plain language do-not list in `content-standards.md`, in the sense that list bans, unless a terminology rule in `docs/product-context.md` requires it or the term is a control's exact on-screen label. A label on something the member only reads is not a control's label and licenses nothing; see check 3
2. The article names the real things rather than a category standing in for them
3. The title, description, opening paragraph, and every heading use only words an agent would say out loud. A term that appears on screen still fails this check in two ways: where the article has adopted a control's label as its own noun instead of naming the control at the point of use, and where the label belongs to something the member only reads and has displaced the word a member would type
...
Check 3 asks three questions of any term that reads as product vocabulary, not one. Is the thing a control the member has to find and act on, or something they only read? Is the word on screen? And would a member use it when the control is not in front of them? The order matters, because a no to the first ends it: a status row, a column heading, or a count is not a control, so its label carries no license and the member's own word wins.

Two cases from September 2026 sit on either side of that first question, and both passed the on-screen test, which is why the on-screen test alone is not enough. "Scope" passed it on the Listings widget migration, September 16: the menu is headed Scope by, so the term looked sourced, and the draft went on to call the control "the scope button", which nothing on screen calls it. The label was real and the license was narrower than the draft assumed. "Off market" passed it on the Dashboard FAQ migration, September 23: the Hot Sheet widget renders the row that way, so the term looked sourced, and the draft put it in a heading against nine other articles and the live article's own heading, all of which write "off-market". The label was real and licensed nothing, because a row in a chart is not a control. `content-standards.md` holds the full rule under "The exact-label rule reaches controls, not readouts".

Only check 1 is greppable, and it over-flags: "public records" and "return to the Dashboard" are both legitimate and both appear in finished articles. "tiles" inside alt text was a third until September 18, 2026, when the exception was struck; a hit there is now a real hit unless the word is a control's on-screen label. Checks 2 to 5 are read, the way the navigation-step rule and the possessive-density rule are read. A failure is reported with the terms it flagged and the replacement for each, not as a bare verdict.
